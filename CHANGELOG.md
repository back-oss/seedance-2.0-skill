...
Current active release: **6.7.0**. Older entries below are preserved as release history, not active version guidance.
...
- Recorded that **a newer model line exists**. Seedance 2.5 was announced 2026-06-23 and launched on consumer surfaces 2026-07-31, with API access reported from mid-July, and the 2.0 line gained a reported 4K tier — all of it after this repository's previous verification date of 2026-06-20, which means the skill was silently presenting a superseded picture as current. `api-status` now opens with the 2.5 section, `model-name-map` gains 2.5 and 2.0-4K rows with a standing rule never to normalize 2.5 to 2.0, the root skill's source gate requires establishing the **model line** and not just the surface, and the README says it above the platform paragraph. Every 2.5 claim is labeled `tech-press`, not `confirmed`: the primary ByteDance and Volcengine pages were unreachable in this pass, and `source-registry` records that gap rather than hiding it. The rule that falls out of it: craft transfers across model lines, platform numbers never do.
- Added `assets/masthead-outlines.json` and `scripts/build_masthead_outlines.py`: the masthead's display type as vector outlines, shaped with HarfBuzz kerning from Bodoni Moda (SIL OFL, attribution and instance axes recorded in the file), with optical size tracking the rendered size rather than a fixed instance. The fonts are vendored under `assets/fonts/` with their OFL text so a clean checkout reproduces the committed geometry offline, and the generator carries `--check`. Font files are excluded from installed payloads, which never regenerate.

- Added `references/sync-budget-protocol.md`: the measurement procedure that can honestly fill the two "not separately measured" cells in `audio-guide` — fixed shot conditions matching the existing Dialogue rules, script-verified sentence ladders in morae (ja), syllables (ko), and characters (zh control), a three-defect scoring rule, a 3-of-4 acceptance threshold, recording via the existing generation-run schema, and write-back rules that keep the `[field]` label and per-surface scope. The empty cells now point at the protocol instead of just admitting the gap.
- Ported the salvageable material from the zh/ja/ko legacy bodies in `references/migrated/` into active guidance: composition vocabulary (rule of thirds, negative space, foreground blur, leading lines, layered depth), long-take and dappled-light terms (木漏れ日 / 나뭇잎 사이로 비치는 빛 / 树叶间洒下的斑驳光), and **Aesthetic Registers** sections for all three languages — 間, 侘び寂び, もののあわれ, 幽玄, 粋; 한, 정, 여백의 미, 신명, 사극; 武侠, 仙侠, 国风/水墨, 废土, 烟火气 — each written in the anti-slop decomposition pattern (name the register, then the physical elements that produce it) rather than copied as bare mood words. The archive README now carries a mining record naming what was ported and which three legacy categories were deliberately left quarantined (euphemism tables, "filter shield" framing, platform-term tables) and why.
- Added **Register (文体)** to `vocab/ja`, closing the asymmetry the last two language passes left: Chinese got Script Variant and Korean got Speech Level, while Japanese — whose register system carries the same budget and characterization consequences — got nothing. Documents 敬語 / です・ます体 / 普通体 with mora costs for identical sentences (ありがとう 5 → ありがとうございます 10; 分かった 4 → 分かりました 6 → かしこまりました 8), the first-person pronoun (私/僕/俺/わたくし) as a register axis unique to Japanese, two-speaker asymmetry as relationship encoding, and the sequence-consistency rule. Five `Audio` vocabulary rows declare the register in a prompt, mirroring Korean's, and `audio-guide` now names Japanese as the second register-budget language beside Korean.
- Added a **dialogue format line** to the Japanese and Korean Dialogue Notes, mirroring the Chinese 台词格式 that already existed: speaker + action + quoted line, with 「 」 for Japanese and double quotes plus a declared speech level for Korean — so all three CJK files now show how to separate performance direction from the words to be spoken.
- Stated explicitly in `vocab/ja` and `vocab/ko` that reference tags stay Latin (`@Image1`, never `@画像1` or `@이미지1`): no surface documents translated Japanese or Korean tags, and the localized `@图片1` family belongs to Chinese-UI surfaces only. The byte-for-byte rule always implied this; nothing said it where a ja/ko prompter would look.
- Added **Timeline Template** skeletons to `vocab/ja` and `vocab/ko`, translated from the Chinese community's bracket-timeline pattern and labeled honestly: the structure is field-observed on 即梦/Dreamina, the Japanese and Korean versions are translations of that structure, not independently observed patterns.
- Added a **Multimodal Template** to `vocab/zh` for parity with ja/ko, in both the `@图片1` and Latin tag families, with the rule that the two families never mix in one prompt.
- Made the register features discoverable from their skill wrappers: `seedance-vocab-ja` gained a Register Rule, `seedance-vocab-ko` a Speech Level Rule, and `seedance-vocab-zh` a Script Variant Rule. All three sections previously existed only in the reference files, so an agent that loaded just the wrapper never learned they existed.

- Added **Script Variant (简繁)** to `vocab/zh`, and declared the file's own variant in its header: it is Simplified (zh-Hans) with mainland terminology, which was true but never stated. Separates prompt script from delivery script (mainland-trained surfaces make Simplified the safer prompt language; a 台灣/香港 deliverable still needs Traditional subtitles), warns that one-to-many character mappings make find-and-replace conversion wrong rather than merely foreign (头发 → 頭髮, not 頭發), and lists regional vocabulary differences that survive a correct script conversion (视频/影片, 屏幕/螢幕, 质量/品質). Three `Text` rows declare the delivery variant. `delivery-qc` now carries the variant in its Text preflight row and requires the `LANG` field of the naming pattern to distinguish `ZH-HANS` from `ZH-HANT`, since the variant is not recoverable from the video.
...
- Added `scripts/prompt_architecture_stress.py` and `evals/prompt-architecture-stress.json`: 102 prompts covering 34 briefs across every mode, written three ways — untrained/listicle style, the shape the beginner example demonstrated, and the `seedance-prompt` Director Formula — and scored mechanically on opening authority, length fit, slop density, craft coverage, structure, and reference integrity. The Director Formula arm scores 3.92 against the `eval-rubric` bar; the other two fail it. Coverage is mode-aware — for modes that build on supplied footage, explicitly preserving a dimension addresses it as fully as specifying one, because demanding a fresh light source from an edit prompt would score the skill's own advice as a defect. Wired into CI with `--strict`, so a change that degrades the doctrine arm fails the build instead of reaching users as "the prompts feel average".
- Added a **Position Costs More Than Count** section to `anti-slop-lexicon`: slop is more expensive in an opening clause than in the constraint tail, so the opening belongs to the subject and its action. The rule existed only in `references/migrated/`, which is explicitly not active guidance.
- Added `tests/test_validate_skills_bytecode.py`, covering the tracked-versus-present distinction that the bytecode check now makes.

### Changed

- **Replaced the masthead's live display type with outlines, fixing a design that only existed on Apple hardware.** The stack `Didot, 'Bodoni MT', 'Hoefler Text', Baskerville, 'Palatino Linotype', Georgia, serif` resolves to Didot only on macOS; Windows fell through to Bodoni MT or Palatino Linotype, and Linux — including the machine this repository is built on, which has none of the six — fell all the way to a default system serif. The "high-contrast editorial serif" the design system specifies was what a minority of readers actually saw. This is the same failure the script wordmark was retired for in v6.6.0; the serif stack carried it too and outlasted the fix. The wordmark and tagline are now glyph outlines from Bodoni Moda, so every reader sees identical type and no font needs to be installed. Specification values moved to the monospace stack so that the only serif on the canvas is outlined and nothing can silently fall back.
- Restamped the six freshness-critical references and `source-registry` to 2026-08-01 with what each review actually concluded, rather than bumping dates to satisfy the gate: `platform-surface-matrix` gains a model-line axis, `api-workflow` marks its endpoint values as 2.0-only, `platform-constraints` records that a reported 4K tier does not lift the documented 480p/720p native baseline, `field-observed-tips` notes that patterns observed on 2.0 are hypotheses on 2.5, and `agent-compatibility` states that packaging is deliberately independent of the model line.
...
- Gave the install instructions a first step. Both `README` and all six quickstarts opened with `python scripts/install_codex_skill.py --force`, a command that only runs inside a copy of the repository nothing had told the reader to obtain; `git clone` and the ZIP fallback now come first.
- Documented the installer as client-agnostic rather than Codex-only. It always accepted `--dest`, so Claude Code and every other client had a one-command install and were being sent to "copy the folder manually" instead. Removed `--force` from the first-install command, since it deletes an existing install and a first install has nothing to delete.
- Rewrote the beginner example in all six quickstarts. The "Directed (strong)" prompt was 29 words — under the 40-110 band the root skill states — and opened on `Medium close-up, eye-level`, spending the highest-weight position on framing metadata, which is the inversion `seedance-prompt` warns against when it says to put the subject and primary action first. Replaced with a subject-first prompt in every language plus the explanation of why the order is the lesson. The Chinese, Japanese, and Korean versions state the length rule in characters or syllables rather than words, as their own vocabulary files require.
- Fixed the README's own hero example, which still opened on `Medium close-up, eye-level` after the quickstart fix — the highest-visibility instance of the camera-first inversion, sitting in the "Direct the scene, don't decorate it" section every visitor reads first. Now subject-first, with the ordering called out as part of the direction.
- Documented that installs skip the 18 MB image gallery, so the gallery links in an installed copy's README are expected not to resolve there.
- Hardened `tests/test_install_payload.py` against a pre-existing `.claude/` directory: the in-tree refusal test asserted the directory did not exist, so any contributor running Claude Code in this repository — which creates `.claude/` — got a false test failure. The assertion is now "nothing new appeared". Added `.claude/` to `.gitignore` for the same reason.
- Corrected the stress scorer's description of its `quickstart_style` arm to say the shape is the one the example taught before it was fixed — a frozen regression arm, not current documentation — and replaced an invented `[community, 2026-06]` claim label in `anti-slop-lexicon` with the repository's standard `[field]` label.
- Refused in-tree install destinations instead of attempting them. `shutil.copytree` walks the source tree and the destination is created before the walk reaches it, so `--dest .claude/skills` run from the repository root copied the repository into itself: `.claude/skills/seedance-20/.claude/skills/seedance-20/...`, 757 directories and a 4105-character path before it died on `ENAMETOOLONG`. The guard reports the problem as a message with the correct command rather than a traceback, since this is the first command a new user runs and a stack trace reads as a broken tool. Documented the project-local install as running from the target project by absolute script path, in all six languages.
- Made the installer's closing line client-neutral. It printed "Restart Codex" regardless of `--dest`, so a Claude Code user following the documented command was told to restart software they may not have.
...
- Added a Director's Voice model with six production functional style archetypes so a project keeps one consistent directorial hand, plus a long-form look spine that progresses scale, movement, light, and sound across connected clips and marks the single clip that breaks the pattern at the story turn.
...
- Added a safety fast-path to operating-loop intake: clear safety, IP, likeness, or evasion risks route to the quality gate before any planning (stress-test finding).
